...
- (void) show{
	...
	v.frame = CGRectMake(0, 0, textSize.width + 200, textSize.height + 200);
	...
	CGRect _frame= v.frame;
	float _w = textSize.width + 10;
	float _h = textSize.height + 10;
	float _x = _frame.origin.x + (_frame.size.width - _w)/2;
	float _y = _frame.origin.y + (_frame.size.height - _h)/2;
	
	[label setFrame:CGRectMake(2, 2, textSize.width + 5, textSize.height + 5)];
	label.alpha = 1 ;
	[v setFrame:CGRectMake(_x,_y,_w,_h)];
	v.alpha = 1;
