...
- (void)drawRect:(CGRect)rect {
    ...
    /*写文字*/
    CGContextSetRGBFillColor (context,  1, 1.0, 1.0, 0.6);//设置填充颜色
...
//    CGContextAddArc(context, self.frame.size.width/2-2, self.frame.size.height/2, 80, 0, -M_PI, 0); //添加一个圆
...
//    
//    //填充圆，无边框
//    CGContextAddArc(context, self.frame.size.width/2, self.frame.size.height/2, 80, 0, 2*M_PI, 0); //添加一个圆
//    CGContextDrawPath(context, kCGPathFill);//绘制填充
    
    //画大圆并填充颜
    UIColor*aColor = [UIColor colorWithRed:1 green:1.0 blue:1 alpha:0.3];
    CGContextSetFillColorWithColor(context, aColor.CGColor);//填充颜色
    CGContextSetLineWidth(context, 3.0);//线的宽度
    CGContextAddArc(context, 250*MainScreenSize_W/414, 40, 40*MainScreenSize_W/414, 0, 2*M_PI, 0); //添加一个圆
