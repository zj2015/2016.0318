...
- (void)viewDidLoad
{
    ...
    if (self.videoId) {
        ...
        [self creatAlertViewWithTitle:@"友情提示" contentMsg:@"没有可以播放的视频" cancleBtnTitle:@"OK" otherBtnTitle:@""];
...
- (void)loadHeaderView
{
    self.headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 10, self.overlayView.frame.size.width, 38)];
...
- (void)loadQualityView
{
    ...
    if (![self.videoURL isEqualToString:@""] && self.videoURL != nil) {
        self.shareBtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        
        frame.origin.x = self.headerView.frame.size.width - 45;
        frame.origin.y = self.headerView.frame.origin.y + 5;
        frame.size.width = 35;
        ...
        [self.overlayView addSubview:self.shareBtn];
    }

    self.qualityButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    
    frame.origin.x = self.headerView.frame.size.width - (50+65);
    frame.origin.y = self.headerView.frame.origin.y + 9;
    frame.size.width = 50;
    frame.size.height = 20;
    self.qualityButton.frame = frame;
    self.qualityButton.clipsToBounds = YES;
    self.qualityButton.layer.cornerRadius= 10;
    ...
    frame.origin.x = self.overlayView.frame.size.width - frame.size.width - 60;
...
- (void)loadSubtitleView
{
    ...
    self.subtitleButton.clipsToBounds = YES;
    self.subtitleButton.layer.cornerRadius= 10;
...
- (BOOL)loadMovieSubtitle
{
    ...
    self.movieSubtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(100, MainScreenSize_W - 90, self.overlayView.bounds.size.width - 200, 40)];
...
- (void)loadBackButton
{
    ...
    [self.backButton setTitle:self.videoTitle forState:UIControlStateNormal];
...
- (void)loadScreenSizeView
{
    ...
    self.screenSizeButton.clipsToBounds = YES;
    self.screenSizeButton.layer.cornerRadius= 10;
...
//    self.player.videoId = @"CFB3EEF09268C3219C33DC5901307461";
    self.player.timeoutSeconds = 30;
...
//            [alert show];
            [blockSelf creatAlertViewWithTitle:@"友情提示" contentMsg:message cancleBtnTitle:@"OK" otherBtnTitle:@""];
            return;
        }
...
#pragma mark - 视频分享
-(void)shareBtnClick2:(UIButton *)btn
{
    [UMSocialConfig setSupportedInterfaceOrientations:UIInterfaceOrientationMaskLandscapeRight];
    
    NSString *shareText = nil;
    shareText = [NSString stringWithFormat:@"CC英语，让孩子放心就读10年的英语学校%@",_videoURL];             //分享内嵌文字
    
    //分享图文样式到微信朋友圈显示字数比较少，只显示分享标题
    [UMSocialData defaultData].extConfig.title = @"CC英语";
    [UMSocialData defaultData].extConfig.wechatTimelineData.title = @"CC英语，让孩子放心就读10年的英语学校";
    
    [UMSocialData defaultData].extConfig.wechatSessionData.url = _videoURL;
    [UMSocialData defaultData].extConfig.wechatTimelineData.url = _videoURL;
    [UMSocialData defaultData].extConfig.qqData.url = _videoURL;
    [UMSocialData defaultData].extConfig.qzoneData.url = _videoURL;
    
    UIImage *shareImage = [UIImage imageWithData: [NSData dataWithContentsOfURL:[NSURL URLWithString:_imgUrlstring]]];            //分享内嵌图片
...
-(void)shareBtnClick:(UIButton *)btn
{
    ...
    MKActionView *actionView = [[MKActionView alloc]initWithTitleWithArr:shareList WithBlock:^(int tag) {
        [[UMSocialControllerService defaultControllerService] setShareText:shareText shareImage:shareImage socialUIDelegate:self];
        UMSocialSnsPlatform *snsPlatform = [UMSocialSnsPlatformManager getSocialPlatformWithName:shareList[tag]];
...
//                [alertView show];
                [self creatAlertViewWithTitle:@"成功" contentMsg:@"分享成功" cancleBtnTitle:@"好" otherBtnTitle:@""];
...
//                [alertView show];
                [self creatAlertViewWithTitle:@"失败" contentMsg:@"分享失败" cancleBtnTitle:@"好" otherBtnTitle:@""];
            }
        }];
    }
}
...
- (void)creatAlertViewWithTitle:(NSString *)title contentMsg:(NSString *)message cancleBtnTitle:(NSString *)cancle otherBtnTitle:(NSString *)other
{
    bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, MainScreenSize_H, MainScreenSize_W)];
    bgView.backgroundColor = [UIColor clearColor];
    bgView.alpha = 0.5;
    [self.view addSubview:bgView];
    
    alertView = [[UIView alloc] initWithFrame:CGRectMake((MainScreenSize_H-300*SIZE_TIMES)/2, (MainScreenSize_W-100*SIZE_TIMES)/2, 300*SIZE_TIMES, 100*SIZE_TIMES)];
    alertView.backgroundColor = [UIColor whiteColor];
    alertView.clipsToBounds = YES;
    alertView.layer.cornerRadius = 10;
    [self.view addSubview:alertView];
    
    UILabel *tipsLab = [[UILabel alloc] initWithFrame:CGRectMake(20, 5, 300*SIZE_TIMES - 40, 30)];
    tipsLab.font = [UIFont boldSystemFontOfSize:16];
    tipsLab.textAlignment = NSTextAlignmentCenter;
    tipsLab.text = title;
    [alertView addSubview:tipsLab];
    
    CGSize msgLabSize = [NSString getTextHeight:message maxWidth:300*SIZE_TIMES - 30];
    
    UILabel *msgLab = [[UILabel alloc] initWithFrame:CGRectMake(15, 40, 300*SIZE_TIMES - 30, msgLabSize.height)];
    msgLab.font = [UIFont systemFontOfSize:15];
    msgLab.textAlignment = NSTextAlignmentCenter;
    msgLab.text = message;
    [alertView addSubview:msgLab];
    
    UILabel *lineLab  = [[UILabel alloc] initWithFrame:CGRectMake(0, msgLab.bottom + 10, 300*SIZE_TIMES, 0.5)];
    lineLab.backgroundColor = [UIColor lightGrayColor];
    [alertView addSubview:lineLab];
    
    UIButton *cancleBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    if ([other isEqualToString:@""]) {
        cancleBtn.frame = CGRectMake(0, lineLab.bottom + 10, 300 *SIZE_TIMES, 30);
    }else{
        cancleBtn.frame = CGRectMake(0, lineLab.bottom + 10, 150 *SIZE_TIMES, 30);
        
        UIButton *otherBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        otherBtn.frame = CGRectMake(150 *SIZE_TIMES, lineLab.bottom + 10, 150 *SIZE_TIMES, 30);
        [otherBtn setTitle:other forState:UIControlStateNormal];
        [otherBtn setTitleColor:[UIColor colorWithRed:44/255.0 green:112/255.0 blue:246/255.0 alpha:1] forState:UIControlStateNormal];
        [otherBtn addTarget:self action:@selector(alertOtherClick) forControlEvents:UIControlEventTouchUpInside];
        [alertView addSubview:otherBtn];
    }
    [cancleBtn setTitle:cancle forState:UIControlStateNormal];
    [cancleBtn setTitleColor:[UIColor colorWithRed:44/255.0 green:112/255.0 blue:246/255.0 alpha:1] forState:UIControlStateNormal];
    ...
    alertView.frame = CGRectMake((MainScreenSize_H-300*SIZE_TIMES)/2, (MainScreenSize_W- 100 - msgLabSize.height)/2 - 10, 300*SIZE_TIMES, 100 + msgLabSize.height);
}

- (void)alertOtherClick
{
    // 获取videoId的播放url
    [self loadPlayUrls];
