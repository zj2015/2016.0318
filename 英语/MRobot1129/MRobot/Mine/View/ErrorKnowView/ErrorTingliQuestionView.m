...
#import "DrawerView.h"
@implementation ErrorTingliQuestionView
...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if (!_tingLiButton) {
            ...
            _tingLiButton.backgroundColor = PView_GreenColor;
...
- (void)addTingLiData:(ReadingComModel *)data withPage: (NSDictionary *)page withIndex:(int)index
{
    ...
    if ([[page valueForKey:@"type"] isEqual:@"0"]) {
        _questionView1 = [[ErrorQuestionView alloc]initWithFrame:CGRectMake(0, _topBgView.bottom + 20 , self.width, MainScreenSize_H)];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(tingliQuestionViewDelegateWithUnderStandard:andButton:)]) {
        [self.delegate tingliQuestionViewDelegateWithUnderStandard:qId andButton:btn];
    }
}
